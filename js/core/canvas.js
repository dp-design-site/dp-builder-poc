// Canvas module: drag, resize, selection, guides, serialize/deserialize, align/distribute
import { registry } from './widgets.js';
import { mountInspectorForSelection, clearInspector } from './inspector.js';
...
export function initCanvas() {
  ...
  canvas().addEventListener('pointerdown', (e) => {
    if (e.target !== canvas()) return; // only on empty area
  ...
  window.addEventListener('pointerup', () => {
    ...
    const ml = mr.left - cr.left,
      mt = mr.top - cr.top,
      mrgt = ml + mr.width,
      mb = mt + mr.height;
    const ids = $$('.widget', canvas())
      .filter((w) => {
        const r = getRect(w);
...
export function createFromPalette(
  ...
  const el = def.create();
  el.dataset.id = 'w_' + Math.random().toString(36).slice(2, 9);
  el.dataset.type = type;
  el.style.left = Math.round(x / GRID) * GRID + 'px';
  el.style.top = Math.round(y / GRID) * GRID + 'px';
  el.style.width = (def.defaults.w || 120) + 'px';
  el.style.height = (def.defaults.h || 40) + 'px';
...
function reflectSelection() {
  $$('.widget', canvas()).forEach((w) =>
    w.classList.toggle('selected', state.selection.has(w.dataset.id))
  );
  if (state.selection.size === 1) mountInspectorForSelection();
  else clearInspector();
}
...
// Guides
function hideGuides() {
  $('#guide-v').style.display = 'none';
  $('#guide-h').style.display = 'none';
}
function smartGuides(el, nx, ny) {
  const others = $$('.widget', canvas()).filter((w) => w !== el);
  const r = rectAt(el, nx, ny);
  let sx = nx,
    sy = ny,
    showV = false,
    showH = false,
...
export function serialize() {
  const items = $$('.widget', canvas()).map((el) => {
    ...
    return { id: el.dataset.id, type: t, x, y, w, h, ...props };
  });
...
function applyProps(el, type, props) {
  registry[type].applyProps(el, props);
}

// Align/Distribute API used by buttons
function align(fn) {
  const sel = [...state.selection].map((id) =>
    $(`.widget[data-id="${id}"]`, canvas())
  );
  if (sel.length < 2) return;
  const rects = sel.map(getRect);
  fn(sel, rects);
}
export const alignAPI = {
  'align-left': () =>
    align((sel, rects) => {
      ...
      sel.forEach((w) => (w.style.left = L + 'px'));
    }),
  'align-right': () =>
    align((sel, rects) => {
      const R = Math.max(...rects.map((r) => r.right));
      sel.forEach((w, i) => (w.style.left = R - rects[i].w + 'px'));
    }),
  'align-top': () =>
    align((sel, rects) => {
      const T = Math.min(...rects.map((r) => r.top));
      sel.forEach((w) => (w.style.top = T + 'px'));
    }),
  'align-bottom': () =>
    align((sel, rects) => {
      const B = Math.max(...rects.map((r) => r.bottom));
      sel.forEach((w, i) => (w.style.top = B - rects[i].h + 'px'));
    }),
  'align-vcenter': () =>
    align((sel, rects) => {
      const C = Math.round(
      ...
      sel.forEach(
        (w, i) => (w.style.top = C - Math.round(rects[i].h / 2) + 'px')
      );
    }),
  'align-hcenter': () =>
    align((sel, rects) => {
      ...
      sel.forEach(
        (w, i) => (w.style.left = C - Math.round(rects[i].w / 2) + 'px')
      );
    }),
  'distribute-h': () =>
    align((sel, rects) => {
      ...
      const L = sorted[0].r.left,
        R = sorted.at(-1).r.right;
      const total = sorted.reduce((s, o) => s + o.r.w, 0);
      const gap = (R - L - total) / (sorted.length - 1);
      let x = L;
      sorted.forEach(({ r, i }) => {
        sel[i].style.left = x + 'px';
        x += r.w + gap;
      });
    }),
  'distribute-v': () =>
    align((sel, rects) => {
      ...
      const T = sorted[0].r.top,
        B = sorted.at(-1).r.bottom;
      const total = sorted.reduce((s, o) => s + o.r.h, 0);
      const gap = (B - T - total) / (sorted.length - 1);
      let y = T;
      sorted.forEach(({ r, i }) => {
        sel[i].style.top = y + 'px';
        y += r.h + gap;
      });
    }),
};
