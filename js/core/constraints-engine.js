...
// Минимален двигател за констрайнти: създаване, изтриване и прилагане (еднопроходно).
...
export function createConstraint(aEl, aAnchor, bEl, bAnchor){
  ...
  applyConstraintsFor(c.a.id); // моментално прилагане
  return c.id;
}
...
export function applyConstraintsFor(elId){
  const ids = Array.from(store.byEl.get(elId) || []);
  for (const id of ids){
    const c = store.list.get(id); if(!c) continue;
