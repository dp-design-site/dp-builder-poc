// Inspector module: renders property panels based on widget schemas
...
export function mountInspectorForSelection() {
...
function renderEmpty() {
  ...
  form.innerHTML = `<div class="section"><h3>Selection</h3><div class="rows"><div>Няма избран елемент</div></div></div>`;
}
...
function renderFor(el) {
  ...
  for (const grp of spec.inspector?.groups || []) {
    ...
    for (const [key, ctrl] of grp.rows) {
      ...
      html += `<div class="row"><div>${key}</div>${renderControl(
        key,
        ctrl,
        propVals[key]
      )}</div>`;
...
function applyChange(el, type, key, ctrl) {
  ...
  // Merge and apply
  const current = extractByType(el, type);
  current[key] = value;
  registry[type].applyProps(el, current);
}
